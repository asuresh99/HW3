...
# this is how you define a function in Pyth#on
def unique():
    # this is your index page
    # connect to DB
    cursor, connection = util.connect_to_db(username,password,host,port,database)
    # execute SQL commands
    record = util.run_and_fetch_sql(cursor, "(select distinct fruit_a, fruit_b from basket_a full outer join basket_b on basket_a.fruit_a = basket_b.fruit_b where basket_a.a is NULL or basket_b.b is NULL)")
    if record == -1:
        # you can replace this part with a 404 page
        print('Something is wrong with the SQL command')
    else:
